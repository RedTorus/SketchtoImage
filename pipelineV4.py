...
def main(train=True):
    ...
    SketchySet = get_sketchimageclass('zebra', 'tiger', transform=transform, size=256, rgb=False)
    ...
    if train:
        ...
        for epoch in tqdm(range(N_EPOCHS)):
            ...
            for sketch, photo in train_loader:
                ...
                latent_pred_im=diffusion.sample_from_reverse_process(unet, xt, sampling_steps, model_kwargs={'sketch':latent_sketch}, ddim=DDIM)
                pred_im = photo_var_auto_encoder.decoder(latent_pred_im)
                loss = ((pred_eps - eps) ** 2).mean() + ((photo-pred_im)**2).mean() #+ reconstruction_loss
                #print("recieved loss")
        ...
        unetname= f'{dir_name}/final_modelV4.pth'
        ...
        unet = torch.load(f'{dir_name}/final_modelV4.pth')
    ...
    #pdb.set_trace()
    pred_eps=diffusion.sample_from_reverse_process(unet, xt, diffusion.timesteps, model_kwargs={'sketch':latent_sketches}, ddim=DDIM)
